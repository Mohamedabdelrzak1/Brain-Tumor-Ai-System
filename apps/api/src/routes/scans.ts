...
// Alias to match requested endpoint: GET /api/scans/my-scans
router.get("/my-scans", authMiddleware, async (req, res) => {
  // Reuse the same list logic by setting patientId query for student.
  // We keep behavior consistent: student sees own scans; others can pass filters on GET /api/scans.
  (req.query as any).patientId = (req as any).user.userId;
  res.redirect(307, `/api/scans?patientId=${(req as any).user.userId}`);
});
